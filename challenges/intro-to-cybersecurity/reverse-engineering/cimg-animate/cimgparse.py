...
class RenderState:
    ...
    sprites: dict[int, Directive3CreateSprite]
    ...
    def _render_terminal(self) -> str:
        newline = Pixel.ANSI_FMT.format(r=0, g=0, b=0, ch="\n")
        rows = []
    ...
    def display(self) -> None:
        sys.stdout.write(self._render_terminal())
        sys.stdout.flush()
...
@dataclass
class Directive1RenderFrame(Directive, code=1):
    pixels: tuple[Pixel, ...]
    ...
    def to_bytes(self) -> bytes:
        return b"".join(p.to_bytes() for p in self.pixels)

    def apply(self, state: RenderState, idx: int) -> None:
        expected = state.width * state.height
        if len(self.pixels) != expected:
            raise ValueError(
                f"directive[{idx}] code=1 pixel count mismatch, expected {expected}, got {len(self.pixels)}"
            )
        state.framebuffer = list(self.pixels)


@dataclass
class Directive2RenderPatch(Directive, code=2):
...
@dataclass
class Directive3CreateSprite(Directive, code=3):
    ...
    def apply(self, state: RenderState, idx: int) -> None:
        ...
        actual = len(self.ascii_data)
        if actual != expect:
            raise ValueError(
                f"directive[{idx}] code=3 ascii size mismatch, expected {expect}, got {actual}"
            )
        state.sprites[self.sprite_id] = self


@dataclass
class Directive4RenderSprite(Directive, code=4):
    META_STRUCT: ClassVar = struct.Struct("<BBBBBBBBB")
    ...
    x: int
    y: int
    repeat_x: int  # 在 x 方向上重复多少次绘制
    repeat_y: int  # 在 y 方向上重复多少次绘制
    ...
    def apply(self, state: RenderState, idx: int) -> None:
        ...
        for j in range(self.repeat_y):
            for i in range(self.repeat_x):
                ...
                start_y = self.y + j * sprite.height
                p = 0
                for y in range(sprite.height):
                    for x in range(sprite.width):
                        ch = sprite.ascii_data[p]
                        if ch != self.maskout:
                            state.set_pixel(
                                start_x + x,
                                start_y + y,
                                Pixel(self.r, self.g, self.b, ch),
                            )
                        p += 1


@dataclass
class Directive5LoadSprite(Directive, code=5):
    META_STRUCT: ClassVar = struct.Struct("<BBB255s")
    ...
    def apply(self, state: RenderState, idx: int) -> None:
        with open(self.filepath.decode("utf-8"), "rb") as f:
            blob = f.read()
        state.sprites[self.sprite_id] = Directive3CreateSprite(
...
@dataclass
class Directive6Flush(Directive, code=6):
    def to_bytes(self) -> bytes:
        return b"0"
...
@dataclass
class Directive7Sleep(Directive, code=7):
    META_STRUCT: ClassVar = struct.Struct("<I")
    ...
    def to_bytes(self) -> bytes:
        return self.META_STRUCT.pack(self.milliseconds)

    def apply(self, state: RenderState, idx: int) -> None:
        import time

        time.sleep(self.milliseconds / 10)
...
class CIMGFile:
    ...
    def from_bytes(cls, blob: bytes) -> CIMGFile:
        ...
        return cls(header=header, directives=tuple(directives))
    ...
    @classmethod
    def new_version_4(cls, width: int, height: int, directives: Sequence[Directive]):
        header = CIMGHeader(
            magic=b"cIMG",
            version=4,
            width=width,
            height=height,
            remaining_directives=len(directives),
        )
        return cls(header=header, directives=tuple(directives))
